<?php
  ...
  if(is_user_logged_in()):
    ...
    $comment_field='<p><label class="comment-form-textarea">'. __( 'Answer','vibe' ) . '</label>' . '<textarea id="comment" name="comment" class="form_field" rows="8" ">'.$content.'</textarea></p>';
    ...
    $answers=get_comments(array(
        ...
        if(isset($answers) && is_array($answers) && count($answers)){
          echo '<a class="reset_answer button primary" data-security="'.wp_create_nonce('security'.$post->ID).'">'.__('Reset Answer','vibe').'</a>';
        }
