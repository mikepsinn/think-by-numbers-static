var AnalyzePage = function() {
	...
	var initVariableCard = function()
	{
		...
		jQuery('#selectedVariables').on('click', 'li > .closeButton', function(event) 
		{
			...
			removeSelectedVariable(selectedOriginalName, selectedCategoryName, selectedSource);	
		});
	...
	var addData = function(variable) 
	{
		...
		Quantimodo.getMeasurements(filters, function(measurements) 
		{
			...
			var variableIndicator = jQuery("#selectedVariables [variable='" + variable.originalName + "'][category='" + variable.category + "'][source='" + variable.source + "'] .colorIndicator" );
	...
	var removeSelectedVariable = function(originalVariableName, category, source) 
	{       
		if (typeof(Storage) !== "undefined")
		{
			try 
			{
				...
				for(i = 0; i < storedVariables.length; i++)
				{
					if(storedVariables[i].name == originalVariableName && storedVariables[i].cat == category && (source == null || source == '' || storedVariables[i].src == source))
					{
